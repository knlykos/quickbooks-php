        ...
        QBWebConnectorSvcSoap: {
            serverVersion: function (args) {
                return __awaiter(this, void 0, void 0, function* () {
                    yield logger_1.logger.log('serverVersion()');
                    return { serverVersionResult: 'NodeJS QuickBooks SOAP Server v1.0' };
                });
            },
            clientVersion: function (args) {
                return __awaiter(this, void 0, void 0, function* () {
                    yield logger_1.logger.log(`clientVersion() - ${args.strVersion}`);
            ...
            authenticate: function (args) {
                return __awaiter(this, void 0, void 0, function* () {
                    yield logger_1.logger.log(`authenticate() - User: ${args.strUserName}`);
                    const { strUserName, strPassword } = args;
                    // Fetch user from DB
                    const user = yield db_1.db.select()
                        .from(schema_1.quickbooksUser)
                        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.quickbooksUser.qb_username, strUserName), (0, drizzle_orm_1.eq)(schema_1.quickbooksUser.qb_password, strPassword) // In production, hash passwords!
                    ))
                        .limit(1);
                    if (user.length === 0 || user[0].status !== 'e') {
                        yield logger_1.logger.log(`authenticate() - Login failed for ${strUserName}`);
                        return {
                            authenticateResult: {
                    ...
                    yield db_1.db.insert(schema_1.quickbooksTicket).values({
                    ...
                    const count = yield queue_1.queue.left(strUserName);
                    const status = count > 0 ? '' : 'none'; // Empty string means "use currently open company file"
                    yield logger_1.logger.log(`authenticate() - Login success for ${strUserName}, Ticket: ${ticket}, Work: ${status}`);
                    return {
                        authenticateResult: {
            ...
            sendRequestXML: function (args) {
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    yield logger_1.logger.log('sendRequestXML()', ticket);
                    // Validate Ticket
                    ...
                    // Dequeue next item
                    const item = yield queue_1.queue.dequeue(user);
                    if (!item) {
                        return { sendRequestXMLResult: '' };
                    }
                    // Mark as processing
                    yield queue_1.queue.updateStatus(ticket, item.quickbooks_queue_id, queue_1.QUICKBOOKS_STATUS_PROCESSING, 'Dequeued');
                    yield logger_1.logger.log(`Dequeued action: ${item.qb_action}, Ident: ${item.ident}`, ticket);
            ...
            receiveResponseXML: function (args) {
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    yield logger_1.logger.log(`receiveResponseXML() - HResult: ${hresult}, Msg: ${message}`, ticket);
                    // Validate Ticket
                    const ticketRecord = yield db_1.db.select().from(schema_1.quickbooksTicket).where((0, drizzle_orm_1.eq)(schema_1.quickbooksTicket.ticket, ticket)).limit(1);
                    if (ticketRecord.length === 0) {
                    ...
                    if (!queueId) {
                        const processingItem = yield queue_1.queue.processing(user);
                        if (processingItem) {
                    ...
                    if (queueId) {
                        ...
                        let status = queue_1.QUICKBOOKS_STATUS_SUCCESS;
                        let logMsg = 'Success';
                        if (message || (hresult && hresult !== '')) {
                            status = queue_1.QUICKBOOKS_STATUS_ERROR;
                        ...
                        yield queue_1.queue.updateStatus(ticket, queueId, status, logMsg);
                    }
                    // Calculate progress
                    const left = yield queue_1.queue.left(user);
                    ...
                    if (left > 0 && percentage === 100)
                        percentage = 99;
                    yield logger_1.logger.log(`Progress: ${percentage}%`, ticket);
            ...
            connectionError: function (args) {
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    yield logger_1.logger.log(`connectionError() - ${hresult}: ${message}`, ticket);
            ...
            getLastError: function (args) {
                return __awaiter(this, void 0, void 0, function* () {
                    const { ticket } = args;
                    yield logger_1.logger.log('getLastError()', ticket);
                    const ticketRecord = yield db_1.db.select().from(schema_1.quickbooksTicket).where((0, drizzle_orm_1.eq)(schema_1.quickbooksTicket.ticket, ticket)).limit(1);
            ...
            closeConnection: function (args) {
                return __awaiter(this, void 0, void 0, function* () {
                    const { ticket } = args;
                    yield logger_1.logger.log('closeConnection()', ticket);
